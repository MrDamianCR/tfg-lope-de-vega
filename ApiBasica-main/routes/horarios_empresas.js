...
router.post("/horarios_empresas/empresa/:id_empresa", (req, res) => {
...
router.post("/horarios_empresas/borrar/:id_horario", (req, res) => {
    const idHorario = req.params.id_horario;

    const sql = "DELETE FROM horarios_empresas WHERE id_horario = ?";

    connection.query(sql, [idHorario], (error, result) => {
        if (error) {
